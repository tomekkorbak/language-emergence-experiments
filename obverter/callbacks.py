...
class EarlyStopperAccuracy(EarlyStopperAccuracy):
    ...
    def should_stop(self) -> bool:
        if len(self.train_stats) < self.delay:
            return False
        assert self.trainer.train_data is not None, 'Validation data must be provided for early stooping to work'
        return all(logs[self.field_name] > self.threshold for _, logs in self.train_stats[-self.delay:])
